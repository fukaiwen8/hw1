...
```{R}
num_greater_than_zero <- sum(my_vec_bool)
...
:::
```{R}
matrix_3x3 <- matrix(c(1, 2, 3, 4, 5, 6, 7, 8, 9), nrow = 3, byrow = TRUE)
matrix_1x100 <- matrix((1:100)^2, nrow = 1)
print(matrix_3x3)
print(matrix_1x100)
```
...
I expect col wise scan would be faster than row wise scan because matrix is stored in column.
...
time_100_row <- time_scan(row_wise_scan, M_100)
time_100_col <- time_scan(col_wise_scan, M_100)

time_1000_row <- time_scan(row_wise_scan, M_1000)
time_1000_col <- time_scan(col_wise_scan, M_1000)

time_5000_row <- time_scan(row_wise_scan, M_5000)
time_5000_col <- time_scan(col_wise_scan, M_5000)

results <- list(
    "100x100" = list(row_wise = time_100_row, col_wise = time_100_col),
    "1000x1000" = list(row_wise = time_1000_row, col_wise = time_1000_col),
    "5000x5000" = list(row_wise = time_5000_row, col_wise = time_5000_col)
)

results

```
I would say as the matrixs gets bigger and bigger the time difference will be more significant. But col wise scan would always be faster than row wise scan.
